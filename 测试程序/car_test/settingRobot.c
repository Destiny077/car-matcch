...
void settingRobot(double wheel2R, double robotD, double sum_encoders)
{
    ...
    // 配置M1电机 (前左)
    SetMotorConstSpeedValue(_M1_, g_motorencoders, 1.5, 1.2, 0.001);
    
    // 配置M2电机 (前右)
    SetMotorConstSpeedValue(_M2_, g_motorencoders, 1.5, 1.2, 0.001);
    
    // 配置M3电机 (后左)
    SetMotorConstSpeedValue(_M3_, g_motorencoders, 1.5, 1.2, 0.001);
    
    // 配置M4电机 (后右)
    SetMotorConstSpeedValue(_M4_, g_motorencoders, 1.5, 1.2, 0.001);
}
#endif
