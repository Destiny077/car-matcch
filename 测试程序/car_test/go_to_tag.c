 ...
 */
void go_to_tag(double targetX, double targetY, double targetA, unsigned long timeout_ms)
{
    // ========== 获取外部PID控制器 ==========
    ...
    while (1) {
        // ========== 1. 获取当前传感器数据（从AI摄像头) ==========
        double curX = GetAICam(7, 1, 2);    // 获取X坐标
        double curY = GetAICam(7, 1, 3);    // 获取Y坐标
        double curA = GetAICam(7, 1, 5);    // 获取当前方向角度
        ...
        {
            // 保持极小的前进速度，防止停滞
            SetMotorConstSpeed(_M1_, 5);
            SetMotorConstSpeed(_M2_, 5);
            SetMotorConstSpeed(_M3_, 5);
            SetMotorConstSpeed(_M4_, 5);
            continue;   // 跳到下一次循环
        }
        ...
        while (errA > 180)  errA -= 360;
        while (errA < -180) errA += 360;
        
        // ========== 4. 超时保护 ==========
        if ((HAL_GetTick() - startTime) > timeout_ms) break;
        
        // ========== 5. 旋转(W)轴: PID控制 ==========
        // 使用PID使机器人对齐目标方向
        double vw = UpdatePID(&pidW, errA);
    ...
    SetDisplayString(8, "完成追踪", 0xFFE0, 0x0000);
    // ========== 14. 导航完成，停止所有电机 ==========
    speed_control(0, 0);
}
#endif
