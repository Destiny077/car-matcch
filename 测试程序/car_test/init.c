...
void init()
{
    ...
    setup_kinematics(100, 106, 85, 160);
    
    // ========== 驱动机械臂到初始位置 ==========
    // 将机械臂运动学逆解算结果发送给舵机
    kinematics_move(arm_x, arm_y, arm_z, 2000);
    
    // ========== 初始化其他舵机位置 ==========
    servo_control(4, 1500, 1000);   // 舵机4: 中位
    servo_control(5, 1200, 1000);   // 舵机5: 角度调整
    ...
    while (1)
    {
        ...
        {
            // 根据当前坐标驱动机械臂
            kinematics_move(arm_x, arm_y, arm_z, 1000);
            SetWaitForTime(1);
        }
    }  
}
