...
char go_bmp(int sp, double bmp, int dir)
{
    ...
    while (1)
    {
        ...
        long d1 = encoder_abs_long(vl - code1);
        long d2 = encoder_abs_long(vr - code2);
        long d3 = encoder_abs_long(vll - code3);
        long d4 = encoder_abs_long(vrr - code4);
        ...
        if (dir == 1)
            balance_forward_speed(sp, d1 + d3, d2 + d4);
        
        // 根据不同的方向计算累计距离
        if ( dir == 1 )
        {
            long distance_code = d1 + d2 + d3 + d4;
