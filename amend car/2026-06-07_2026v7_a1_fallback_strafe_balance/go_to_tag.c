...
// ========== PID积分限幅值 ==========
double max_I = ROBOT_PID_INTEGRAL_LIMIT;

static char tag_sample_in_range(double curX, double curY, double curA)
{
...
double UpdatePID(PID_Controller *pid, double error)
{
    ...
    output = CLAMP(output, -ROBOT_PID_OUTPUT_LIMIT, ROBOT_PID_OUTPUT_LIMIT);
...
char go_to_tag(double targetX, double targetY, double targetA, unsigned long timeout_ms, char catch_type)
{
    ...
    // ========== 控制周期常数 ==========
    const double LOOP_PERIOD_SEC = 0.02; // 20ms control period

    if (timeout_ms < 100)
        return 0;
    if (catch_type != 0 && catch_type != 1)
        catch_type = 0;
    ...
    while (1)
    {
        ...
        if (GetAICam(ROBOT_AI_CAM_PORT, 1, 1) == 1 || catch_type == 1)
        {
            ...
            // ========== 检查标签是否丢失 ==========
            // 如果X、Y、A都为0，表示未检测到标签；底盘停住等待，避免继续偏离道路。
            if (catch_type == 0 && !tag_sample_in_range(curX, curY, curA))
            {
                valid_sample_count = 0;
                speed_control(0, 0);
                SetWaitForTime(LOOP_PERIOD_SEC);
                continue;
            }
            ...
            {
                valid_sample_count++;
                if (valid_sample_count < ROBOT_TAG_NAV_CONFIRM_LOOPS)
                {
                    speed_control(0, 0);
                    SetWaitForTime(LOOP_PERIOD_SEC);
                    continue;
            ...
            // ========== 6. X轴(左右): 两段速度策略 ==========
            const double DIST_THRESHOLD = ROBOT_TAG_DIST_THRESHOLD; // 远近切换阈值
            const double SPEED_HIGH = ROBOT_TAG_SPEED_HIGH;         // 远距离快移速度
            const double SPEED_LOW = ROBOT_TAG_SPEED_LOW;           // 近距离微调速度
            ...
            vw = CLAMP(vw, -ROBOT_TAG_VW_LIMIT, ROBOT_TAG_VW_LIMIT); // 旋转速度限幅
            ...
            // ========== 12. 到达判定 ==========
            // 当X、Y和角度误差进入可配置容差时，认为已到达
            if (fabs(errX) <= ROBOT_TAG_X_TOLERANCE &&
                fabs(errY) <= ROBOT_TAG_Y_TOLERANCE &&
                fabs(errA) < ROBOT_TAG_A_TOLERANCE)
            {
                reached = 1;
                break;
            }
