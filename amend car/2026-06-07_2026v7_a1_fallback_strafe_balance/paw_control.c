...
{
    if (angle_data < ROBOT_SERVO_MIN_PWM)
        angle_data = ROBOT_SERVO_MIN_PWM;
    if (angle_data > ROBOT_SERVO_MAX_PWM)
        angle_data = ROBOT_SERVO_MAX_PWM;
    if (time < 100)
        time = 100;
    // 调用舵机通用控制函数
    // 参数: 舵机ID(5), 目标角度, 执行时间
    servo_control(ROBOT_SERVO_PAW_ID, angle_data, time);
