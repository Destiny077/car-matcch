...
static char confirm_a1_tag_seen(unsigned long timeout_ms)
{
    ...
    SetDisplayString(8, "WAIT A1", 0xFFE0, 0x0000);
    SetWaitForTime(ROBOT_A1_TAG_WARMUP_SEC);

    while ((GetSysTime() - start_time) <= timeout_ms)
    {
        uint32_t tag_id = GetAICam(ROBOT_AI_CAM_PORT, 1, 1);
        double curX = GetAICam(ROBOT_AI_CAM_PORT, 1, 2);
        double curY = GetAICam(ROBOT_AI_CAM_PORT, 1, 3);
        double curA = GetAICam(ROBOT_AI_CAM_PORT, 1, 5);

        SetDisplayVar(1, tag_id, YELLOW, BLACK);
        SetDisplayVar(3, curX, YELLOW, BLACK);
        SetDisplayVar(5, stable_count, YELLOW, BLACK);
        ...
        else
        {
            stable_count = 0;
            speed_control(0, 0);
            SetDisplayString(8, (tag_id == ROBOT_A1_TAG_ID) ? "A1 DATA WAIT" : "A1 NO ID", 0xF800, 0x0000);
        }
