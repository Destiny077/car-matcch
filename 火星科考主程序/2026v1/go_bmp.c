 ...
 *   当累计位移达到目标距离时，停止电机。
 */
void go_bmp(int sp, double bmp, int dir)
{
    // ========== 获取外部全局变量 ==========
    ...
    while (1)
    {
        ...
        if ( dir != 2 )
        {
            // 方向1、3、4的距离计算
            // 计算四个电机的编码增量平均值作为位移
            if ( (double)abs((vl+vll)-(code1+code3)+(vr+vrr)-(code2+code4)) * rate_encoder/4 >= bmp )
            {
                break;      // 达到目标距离，退出循环
            }
        }
        ...
        {
            // 方向2(平移运动)的距离计算
            // 对于平移运动使用不同的编码组合和校正系数(0.888)
            if ( dir == 2 )
            {
                if ( (double)(abs((code1+code4)-(vl+vrr)+(vr+vll)-(code2+code3)) * rate_encoder/4) * 0.888 >= bmp )
                {
                    break;  // 达到目标距离，退出循环
                }
            }
